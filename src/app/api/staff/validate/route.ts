...
export async function POST(req: Request) {
	try {
		...
		for (const ep of ENDPOINTS) {
			const res = await fetch(`${BASE}${ep}`, {
				...
				headers: {
					...
					"User-Agent": "jal-virtual-event-portal/1.0 (+nextjs)",
				},
				cache: "no-store",
			});
			...
			if (res.ok) {
				// Extract staff name from the response
				const staffName = body?.username || body?.name || body?.callsign || 'Unknown';
		...
		for (const ep of ENDPOINTS) {
			...
			if (res2.ok) {
				// Extract staff name from the response
				const staffName = body2?.username || body2?.name || body2?.callsign || 'Unknown';
